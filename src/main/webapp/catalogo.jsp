...
<main class="container py-5">
    ...
    <form action="<%= request.getContextPath() %>/propiedades.jsp" method="get" class="card filter-panel border-0 shadow-sm p-3 p-lg-4 mb-5">
        ...
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-2 d-flex gap-2"><button class="btn btn-accent flex-grow-1" type="submit">Filtrar</button><a class="btn btn-outline-secondary btn-clear-filter" href="<%= request.getContextPath() %>/propiedades.jsp" title="Quitar filtros">Quitar</a></div>
        </div>
    </form>
    ...
    <div class="row g-4">
        <% if (propiedades != null && !propiedades.isEmpty()) { for (Property propiedad : propiedades) { %>
            <div class="col-md-6 col-xl-4"><article class="card property-card h-100 border-0 shadow-sm overflow-hidden"><a class="property-image-link" href="<%= request.getContextPath() %>/propiedad.jsp?id=<%= propiedad.getId() %>" aria-label="Ver detalle de <%= propiedad.getTitle() %>"><div class="ratio ratio-16x9 bg-brand"><img src="<%= propiedad.getImageUrl() == null ? "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=900&q=80" : propiedad.getImageUrl() %>" class="object-fit-cover" alt="<%= propiedad.getTitle() %>"></div></a><div class="card-body"><span class="badge text-bg-light mb-2"><%= propiedad.getOperation() %></span><h3 class="h5"><%= propiedad.getTitle() %></h3><p class="text-secondary mb-2"><%= propiedad.getCity() %> · <%= propiedad.getBedrooms() %> habitaciones · <%= propiedad.getBathrooms() %> baños</p><p class="fw-bold text-brand mb-3"><%= precioLegible(propiedad.getPrice()) %> · <%= propiedad.getArea().toPlainString() %> m²</p><a class="btn btn-sm btn-accent w-100 mb-2" href="<%= request.getContextPath() %>/propiedad.jsp?id=<%= propiedad.getId() %>">Ver detalle</a><% if (session.getAttribute("usuarioId") != null) { %><form action="<%= request.getContextPath() %>/app/favoritos" method="post"><input type="hidden" name="accion" value="agregar"><input type="hidden" name="idPropiedad" value="<%= propiedad.getId() %>"><button class="btn btn-sm btn-outline-dark w-100" type="submit">♡ Agregar a favoritos</button></form><% } %></div></article></div>
