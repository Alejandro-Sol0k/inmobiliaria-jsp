...
<main class="container py-5">
    ...
    <form action="<%= request.getContextPath() %>/propiedades.jsp" method="post" class="card filter-panel border-0 shadow-sm p-3 p-lg-4 mb-5">
        ...
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-2 d-flex gap-2"><button class="btn btn-accent flex-grow-1" type="submit">Filtrar</button><button class="btn btn-outline-secondary btn-clear-filter" type="submit" name="accion" value="limpiar" title="Quitar filtros">Quitar</button></div>
        </div>
    </form>
    ...
    <div class="row g-4">
        <% if (propiedades != null && !propiedades.isEmpty()) { for (Property propiedad : propiedades) { %>
            <div class="col-md-6 col-xl-4"><article class="card property-card h-100 border-0 shadow-sm overflow-hidden"><form action="<%= request.getContextPath() %>/propiedad.jsp" method="post" class="property-detail-form"><input type="hidden" name="id" value="<%= propiedad.getId() %>"><button class="property-image-link" type="submit" aria-label="Ver detalle de <%= propiedad.getTitle() %>"><img src="<%= propiedad.getImageUrl() == null || propiedad.getImageUrl().trim().isEmpty() ? "https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=900&q=80" : propiedad.getImageUrl() %>" class="object-fit-cover" alt="<%= propiedad.getTitle() %>"></button></form><div class="card-body"><span class="badge text-bg-light mb-2"><%= propiedad.getOperation() %></span><h3 class="h5"><%= propiedad.getTitle() %></h3><p class="text-secondary mb-2"><%= propiedad.getCity() %> · <%= propiedad.getBedrooms() %> habitaciones · <%= propiedad.getBathrooms() %> baños</p><p class="fw-bold text-brand mb-3"><%= precioLegible(propiedad.getPrice()) %> · <%= propiedad.getArea().toPlainString() %> m²</p><form action="<%= request.getContextPath() %>/propiedad.jsp" method="post" class="mb-2"><input type="hidden" name="id" value="<%= propiedad.getId() %>"><button class="btn btn-sm btn-accent w-100" type="submit">Ver detalle</button></form><% if (session.getAttribute("usuarioId") != null) { %><form action="<%= request.getContextPath() %>/app/favoritos.jsp" method="post"><input type="hidden" name="accion" value="agregar"><input type="hidden" name="idPropiedad" value="<%= propiedad.getId() %>"><button class="btn btn-sm btn-outline-dark w-100" type="submit">♡ Agregar a favoritos</button></form><% } %></div></article></div>
