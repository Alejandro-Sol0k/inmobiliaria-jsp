...
<main class="container py-5">
    ...
    <% if ("ok".equals(request.getParameter("actualizado"))) { %><div class="alert alert-success">Tus favoritos fueron actualizados.</div><% } %>
    <% if (request.getParameter("error") != null || request.getAttribute("errorFavoritos") != null) { %><div class="alert alert-danger">No fue posible actualizar tus favoritos.</div><% } %>
    <div class="row g-4">
        <% if (favoritos != null && !favoritos.isEmpty()) { for (Property propiedad : favoritos) { %>
            <div class="col-md-6 col-xl-4"><article class="card property-card h-100 border-0 shadow-sm overflow-hidden"><div class="ratio ratio-16x9 bg-brand"><img src="<%= propiedad.getImageUrl() == null ? request.getContextPath() + "/assets/images/altaltium-logo.png" : propiedad.getImageUrl() %>" class="object-fit-cover" alt="<%= propiedad.getTitle() %>"></div><div class="card-body d-flex flex-column"><span class="badge text-bg-light align-self-start mb-2"><%= propiedad.getOperation() %></span><h2 class="h5"><%= propiedad.getTitle() %></h2><p class="text-secondary mb-2"><%= propiedad.getCity() %> · <%= propiedad.getBedrooms() %> habitaciones · <%= propiedad.getBathrooms() %> baños</p><p class="fw-bold text-brand"><%= precioLegible(propiedad.getPrice()) %></p><form action="<%= request.getContextPath() %>/app/favoritos" method="post" class="mt-auto"><input type="hidden" name="accion" value="quitar"><input type="hidden" name="idPropiedad" value="<%= propiedad.getId() %>"><button class="btn btn-sm btn-outline-danger" type="submit">Quitar de favoritos</button></form></div></article></div>
        <% } } else { %><div class="col-12"><div class="empty-state text-center p-5"><div class="empty-state-icon">♡</div><h2 class="h5">Todavía no tienes favoritos</h2><p class="text-secondary">Explora el catálogo y guarda las propiedades que te interesen.</p><a class="btn btn-accent" href="<%= request.getContextPath() %>/propiedades">Explorar propiedades</a></div></div><% } %>
    </div>
</main>
