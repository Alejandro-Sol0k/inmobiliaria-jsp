...
<main class="container py-5">
    ...
    <div class="row g-4">
        <% if (favoritos != null && !favoritos.isEmpty()) { for (Property propiedad : favoritos) { %>
            <div class="col-md-6 col-xl-4"><article class="card property-card h-100 border-0 shadow-sm overflow-hidden"><a class="property-image-link" href="<%= request.getContextPath() %>/propiedad?id=<%= propiedad.getId() %>" aria-label="Ver detalle de <%= propiedad.getTitle() %>"><div class="ratio ratio-16x9 bg-brand"><img src="<%= propiedad.getImageUrl() == null ? request.getContextPath() + "/assets/images/altaltium-logo.png" : propiedad.getImageUrl() %>" class="object-fit-cover" alt="<%= propiedad.getTitle() %>"></div></a><div class="card-body d-flex flex-column"><span class="badge text-bg-light align-self-start mb-2"><%= propiedad.getOperation() %></span><h2 class="h5"><%= propiedad.getTitle() %></h2><p class="text-secondary mb-2"><%= propiedad.getCity() %> · <%= propiedad.getBedrooms() %> habitaciones · <%= propiedad.getBathrooms() %> baños</p><p class="fw-bold text-brand"><%= precioLegible(propiedad.getPrice()) %></p><div class="d-grid gap-2 mt-auto"><a class="btn btn-sm btn-accent" href="<%= request.getContextPath() %>/propiedad?id=<%= propiedad.getId() %>">Ver detalle</a><form action="<%= request.getContextPath() %>/app/favoritos" method="post"><input type="hidden" name="accion" value="quitar"><input type="hidden" name="idPropiedad" value="<%= propiedad.getId() %>"><button class="btn btn-sm btn-outline-dark w-100" type="submit">Quitar de favoritos</button></form></div></div></article></div>
